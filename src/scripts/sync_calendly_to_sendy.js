...
async function run() {
  ...
  if (rows.length) {
    logger.info(`⏱️ Estimating status check duration ~${Math.ceil(rows.length * 120 / 1000)}s (assuming ~120ms per API call).`);
  }
...
run()
  .then(() => {
    ...
    logger.info('✨ Calendly sync completed successfully.');
    process.exit(0);
  })
  .catch((err) => {
